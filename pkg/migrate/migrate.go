...
func NewMigrator() *Migrator {
	// 初始化必要属性
	migrator := &Migrator{
		Folder:   "orm/migration/",
		DB:       orm.DB,
		Migrator: orm.DB.Migrator(),
	}
...
func (migrator *Migrator) Up() {
	...
	if !last {
		console.Success("orm is up to date.")
	}
}
...
func (migrator *Migrator) rollbackMigrations(migrations []Migration) bool {
	...
	for _, _migration := range migrations {
		...
		if mFile.Down != nil {
			mFile.Down(orm.DB.Migrator(), orm.SqlDB)
		}
...
func (migrator *Migrator) readAllMigrationFiles() []MigrationFile {
	...
	for _, f := range files {
		// 去除文件后缀 .go
		fileName := helper.FileNameWithoutExtension(f.Name())
...
func (migrator *Migrator) runUpMigration(mFile MigrationFile, batch int) {
	...
	if mFile.Up != nil {
		...
		// 执行 up 方法
		mFile.Up(orm.DB.Migrator(), orm.SqlDB)
		// 提示已迁移了哪个文件
...
func (migrator *Migrator) Fresh() {
	// 获取数据库名称，用以提示
	dbname := orm.CurrentDatabase()

	// 删除所有表
	err := orm.DeleteAllTables()
	console.ExitIf(err)
	console.Success("clear orm " + dbname)
